...
class XLSXWriter
{
	...
	protected function initializeSheet($sheet_name, $col_props = array(), $auto_filter = false, $freeze_rows = false, $freeze_columns = false)
	{
		...
		$i = 0;
		if (!empty($col_props)) {
			foreach ($col_props as $col_prop) {
				$width = isset($col_prop['width']) ? floatval($col_prop['width']) : 12.1;
				$collapsed = isset($col_prop['collapsed']) && $col_prop['collapsed'] ? "collapsed='true'" : "collapsed='false'";
				$hidden = isset($col_prop['hidden']) && $col_prop['hidden'] ? "hidden='true'" : "hidden='false'";
				$sheet->file_writer->write('<col ' . $collapsed . ' ' . $hidden . ' max="' . ($i + 1) . '" min="' . ($i + 1) . '" style="0" customWidth="true" width="' . $width . '"/>');
				$i++;
			}
		}
	...
	{
		if (empty($sheet_name) || !empty($this->sheets[$sheet_name]))
			return;
		...
		$col_props = isset($col_options['col_props']) ? (array)$col_options['col_props'] : array();
		...
		self::initializeSheet($sheet_name, $col_props, $auto_filter, $freeze_rows, $freeze_columns);
		$sheet = &$this->sheets[$sheet_name];
		$sheet->columns = $this->initializeColumnTypes($header_types);
		if (!$suppress_row) {
			$header_row = array_keys($header_types);

			$ht = isset($col_options['height']) ? floatval($col_options['height']) : 16.1;
			$sheet->file_writer->write('<row collapsed="false" customFormat="false" customHeight="true" hidden="false" ht="' . $ht . '" outlineLevel="0" r="' . (1) . '">');
	...
	public function writeSheetRow($sheet_name, array $row, $row_options = null)
	{
		...
		if (!empty($row_options)) {
			$ht = isset($row_options['height']) ? floatval($row_options['height']) : 16.1;
			$customHt = true;
			...
			$sheet->file_writer->write('<row collapsed="false" customFormat="false" customHeight="false" hidden="false" ht="16.1" outlineLevel="0" r="' . ($sheet->row_count + 1) . '">');
		}
	...
	protected function writeCell(XLSXWriter_BuffererWriter &$file, $row_number, $column_number, $value, $num_format_type, $cell_style_idx)
	{
		...
		} elseif ($num_format_type == 'n_numeric' && strpos($value, ":") === false) {
	...
	protected function styleFontIndexes()
	{
		...
		$default_font = array('size' => '11', 'name' => 'Calibri', 'family' => '2');
	...
	protected function writeStylesXML()
	{
		...
		$file->write('<font><name val="Calibri"/><charset val="1"/><family val="2"/><sz val="11"/></font>');
		$file->write('<font><name val="Calibri"/><family val="0"/><sz val="11"/></font>');
		$file->write('<font><name val="Calibri"/><family val="0"/><sz val="11"/></font>');
		$file->write('<font><name val="Calibri"/><family val="0"/><sz val="11"/></font>');
	...
	private static function numberFormatStandardized($num_format)
	{
		...
		for ($i = 0, $ix = strlen($num_format); $i < $ix; $i++) {
			...
			if ($ignore_until == '' && $c == '[')
				...
				$ignore_until = '';
			if ($ignore_until == '' && ($c == '-'  || $c == '('  || $c == ')') && ($i == 0 || $num_format[$i - 1] != '_'))
				$escaped .= "\\" . $c;
			else
				$escaped .= $c;
		}
